using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace FreeNet
{
    class Defines   // public을 지정하지 않으면, internal 클래스임. 동일한 어셈블리 내에서만 볼 수 있음.
    {
        public static readonly short HEADERSIZE = 2;    // public클래스가 아니므로, Defines.HEADERSIZE로 접근해야 하는 듯.
    }


    // 패킷에서 헤더를 byte[] 배열 message_buffer에 복사해, 본문의 크기를 알아낸 다음 (BitConverter.ToInt16(message_buffer, startIndex);
    // 읽어야 할 크기에, 알아낸 본문의 크기 - HEADERSIZE를 설정 후, 건네받은 원본 바이트 배열을 다 읽어서 저장
    // 등록한 콜백 함수에 복사한 배열을 매개변수로 담아 실행


    // 바이트를 패킷 형식으로 해석해주는 해석기
    /// <summary>
    /// [header][body] 구조를 갖는 데이터를 파싱하는 클래스.
    /// - header : 데이터 사이즈. Defines.HEADERSIZE에 정의된 타입만큼의 크기를 갖는다.
    ///            2바이트일 경우 Int16, 4바이트는 Int32로 처리하면 된다.
    ///            본문의 크기가 Int16.Max값을 넘지 않는다면 2바이트로 처리하는 것이 좋을 것 같다. 1바이트 = FF 2바이트 = FFFF
    /// - body : 메시지 본문.
    /// 패킷에서 헤더를 byte[] 배열 message_buffer에 복사해, 본문의 크기를 알아낸 다음 (BitConverter.ToInt16(message_buffer, startIndex);
    /// 읽어야 할 크기에, 알아낸 본문의 크기 - HEADERSIZE를 설정 후, 건네받은 원본 바이트 배열을 다 읽어서 저장
    /// 등록한 콜백 함수에 복사한 배열을 매개변수로 담아 실행
    /// </summary>
    class CMessageResolver
    {
        // 바이트 배열을 매개변수로 취하는 대리자 정의
        // Const<T>.Value는 private set으로 설정되는데, 생성자에서만 설정되고, .Value로 접근되므로, Const처럼 사용 됨.
        // Const<T>는 struct이므로, 값으로 전달됨. struct에 포함된 배열의 경우엔? 얕은 복사로 전달되나?
        public delegate void CompletedMessageCallback(Const<byte[]> buffer);

        // 메시지 사이즈
        int message_size;

        byte[] message_buffer = new byte[1024]; // 진행중인 버퍼 (원본 버퍼에서 읽어들여 저장할 대상 버퍼)
        int current_position;                   // 버퍼에 기록할 현재 위치.(대상 버퍼의 인덱스) // 패킷 하나를 완성한 뒤에는 0으로 초기화 시켜줘야 한다.
        int position_to_read;                   // 읽어와야 할 목표 위치.
        int remain_bytes;                       // 읽어 올 남은 바이트 (읽어야할 전체 위치 - 현재 위치)

        // 생성자
        public CMessageResolver()
        {
            this.message_size = 0;
            this.current_position = 0;
            this.position_to_read = 0;
            this.remain_bytes = 0;
        }

        /// <summary>
        /// 목표지점으로 설정된 위치까지의 바이트를 원본 버퍼로부터 복사한다.
        /// 데이터가 모자랄 경우 현재 남은 바이트까지만 복사한다.
        /// </summary>
        /// <param name="buffer"></param>
        /// <param name="offset"></param>
        /// <param name="transferred"></param>
        /// <param name="size_to_read"></param>
        /// <returns>다 읽었으면 true, 데이터가 모자라서 못 읽었으면 false를 리턴</returns>
        // 전달되는 buffer와 src_position외에는 다 이객체의 멤버인데,
        // buffer의 원본 인덱스에서 이 객체 멤버 message_buffer의 current_position에 읽어야할 크기 -  현재 대상 인덱스를 뺀 크기만큼
        // (읽어야할 남은 크기가 더 적으면 그 크기만큼) 복사. 복사 후, 원본 인덱스는 복사한 크기만큼 더해짐
        bool Read_Until(byte[] buffer, ref int src_position, int offset, int transferred)
        {
            if(this.current_position >= offset + transferred)
            {
                // 들어온 데이터만큼 다 읽은 상태이므로 더 이상 읽을 데이터가 없다.
                return false;
            }

            // 읽어와야 할 바이트
            // 데이터가 분리되어 올 경우, 이전에 읽어놓은 값을 빼줘서 부족한 만큼 읽어올 수 있도록 계산해 준다.
            // position_to_read는 읽을 인덱스 다음을 가리켜야 함. 예) 읽을 인덱스가 0이면 1을 가리켜야 함.
            int copy_size = this.position_to_read - this.current_position;

            // 남은 데이터가 더 적다면 가능한 만큼만 복사한다.
            if(this.remain_bytes < copy_size)
            {
                copy_size = this.remain_bytes;
            }

            // 버퍼에 복사. (원본배열, 인덱스, 대상배열, 인덱스, 길이)
            Array.Copy(buffer, src_position, this.message_buffer, this.current_position, copy_size);

            // 원본 버퍼 포지션 이동
            src_position += copy_size;

            // 타겟 버퍼 포지션도 이동
            this.current_position += copy_size;

            // 남은 바이트 수
            this.remain_bytes -= copy_size;

            // 목표지점까지 도달 못했으면 false
            if(this.current_position < this.position_to_read)
            {
                return false;
            }

            return true;
        }

        /// <summary>
        /// 소켓 버퍼로부터 데이터를 수신할 때마다 호출된다.
        /// 데이터가 남아 있을 때까지 계속 패킷을 만들어 callback을 호출해 준다.
        /// 하나의 패킷을 완성하지 못했다면 버퍼에 보관해 놓은 뒤 다음 수신을 기다린다.
        /// </summary>
        /// <param name="buffer">복사해올 원본 버퍼</param>
        /// <param name="offset">원본 버퍼의 위치를 조절할 오프셋. 패킷이 뭉쳐 올 경우, 원본 버퍼의 포지션은 계속 앞으로 가야하는 데, 그 때 쓰임</param>
        /// <param name="transferred">전달받은 이번에 읽어올(남은) 바이트 수</param>
        /// <param name="callback">패킷을 전부 이 객체의 버퍼(message.buffer)에 복사했을 때, 복사한 대상 버퍼를 매개변수로 실어 대리자에서 실행할 메서드</param>
        public void On_Receive(byte[] buffer, int offset, int transferred, CompletedMessageCallback callback)
        {
            // 이번 receive로 읽어오게 될 바이트 수
            this.remain_bytes = transferred;

            // 원본 버퍼의 포지션값
            // 패킷이 여러개 뭉쳐 올 경우, 원본 버퍼의 포지션은 계속 앞으로 가야 하는데, 그 처리를 위한 변수이다.
            int src_position = offset;

            // 남은 데이터가 있다면 계속 반복한다.
            while(this.remain_bytes > 0)
            {
                bool completed = false;

                // 헤더만큼 못 읽은 경우, 헤더를 먼저 읽는다.
                if(this.current_position < Defines.HEADERSIZE)
                {
                    // 읽어야 할 목표 지점을 헤더 위치까지로 설정(헤더 위치까지 도달하도록 설정)
                    this.position_to_read = Defines.HEADERSIZE;

                    // buffer의 src_position에서 이 객체의 멤버 this.buffer의 current_position에 (읽어야할 전체 위치 - 현재 위치: 남은 바이트 수)만큼 복사한다.
                    completed = Read_Until(buffer, ref src_position, offset, transferred);
                    if(!completed)
                    {
                        // 아직 다 못읽었으므로, 다음 receive를 기다린다.
                        return;
                    }

                    // 헤더 하나를 온전히 읽어왔으므로 메시지 사이즈를 구한다.
                    // 헤더에 본문의 메시지 사이즈가 기록되어 있는데,
                    // 헤더에서 읽어온 데이터가 message_buffer에 바이트 배열로 복사하였으므로, 바이트 배열을 기본 데이터 형으로 변환하여, 본문의 크기를 구한다.
                    this.message_size = Get_Body_Size(); // 헤더 사이즈가 2바이트 short이므로, BitConverter.ToInt16(this.message_buffer, 0);

                    // 다음 목표 지점(헤더 + 메시지 사이즈)
                    this.position_to_read = this.message_size + Defines.HEADERSIZE;

                }

                // 메시지를 읽는다.
                completed = Read_Until(buffer, ref src_position, offset, transferred);

                if(completed)
                {
                    // 패킷 하나를 완성 했다. 콜백 함수에 담아 호출.
                    callback(new Const<byte[]>(this.message_buffer));
                    Clear_Buffer(); // message_buffer를 0으로 1024크기만큼 채우고, 현재의 위치 0, 메시지 크기 0으로 재설정.
                }
            }
        }

        int Get_Body_Size()
        {

            switch (Defines.HEADERSIZE)
            {
                case 2:
                    return BitConverter.ToInt16(this.message_buffer, 0);    // 바이트배열 message_buffer를 Int16으로 변환
                case 4:
                    return BitConverter.ToInt32(this.message_buffer, 0);
                default:
                    return BitConverter.ToInt32(this.message_buffer, 0);    // 얼마든지 어차피 배열에 담긴 값은 Int32면 다 담기지 않을까? 나중에 혹시 이걸 실제로 구현한다면 테스트 하겠지.
            }

            #region 원본
            // 헤더 타입의 바이트만큼을 읽어와 메시지 사이즈를 리턴한다.
            Type type = Defines.HEADERSIZE.GetType(); // short이므로, Int16이 반환됨.
            // 코딩 오류가 아닐까? HEADERSIZE = 4일 경우엔, Int32로 변환해야 할 것이다. 4바이트를 읽을 것이므로, 하지만, 본문이 32767바이트 이상일 경우는 없을 듯하므로, Int16만쓰일지도,
            if (type.Equals(typeof(Int16)))
            {
                return BitConverter.ToInt16(this.message_buffer, 0);
            }

            return BitConverter.ToInt16(this.message_buffer, 0);
            #endregion 원본. 끝.
        }


        void Clear_Buffer()
        {
            //                     array    index        length
            Array.Clear(this.message_buffer, 0, this.message_buffer.Length);
            this.current_position = 0;
            this.message_size = 0;
        }











    }
}
